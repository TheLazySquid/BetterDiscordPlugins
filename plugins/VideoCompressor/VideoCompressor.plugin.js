...
var attachFiles = [...BdApi.Webpack.getWithKey(Filters.byStrings("filesMetadata:", "requireConfirm:"))];
var [premiumPermissions, ModalSystemMangled, ModalMangled] = BdApi.Webpack.getBulk(
  ...
  async add(videoSample, shouldClose, encodeOptions) {
    try {
      ...
      if (this.customEncoder) {
        ...
        const promise = this.customEncoderCallSerializer.call(() => this.customEncoder.encode(clonedSample, finalEncodeOptions)).then(() => this.customEncoderQueueSize--).catch((error2) => this.encoderError ??= error2).finally(() => {
          clonedSample.close();
        });
  ...
  async add(audioSample, shouldClose) {
    try {
      ...
      if (this.customEncoder) {
        ...
        const promise = this.customEncoderCallSerializer.call(() => this.customEncoder.encode(clonedSample)).then(() => this.customEncoderQueueSize--).catch((error2) => this.encoderError ??= error2).finally(() => {
          clonedSample.close();
        });
        if (this.customEncoderQueueSize >= 4) {
          await promise;
...
function warning(message, showName = true) {
...
mediabunny/dist/modules/src/wave/riff-reader.js:
  (*!
   * Copyright (c) 2025-present, Vanilagy and contributors
   *
   * This Source Code Form is subject to the terms of the Mozilla Public
